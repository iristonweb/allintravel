...
export async function ensurePlatformSchema(): Promise<void> {
  ...
  const run = async () => {
    ...
    await db.execute(sql`ALTER TABLE trips ADD COLUMN IF NOT EXISTS marketplace_snapshot jsonb`);
    await db.execute(sql`
  ...
  await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("ensurePlatformSchema timeout")), 8_000),
    ),
  ]);
}
