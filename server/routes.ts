...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/chat/:room", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const messages = since
        ? await storage.getChatMessages(room, Number(limit), String(since))
        : await storage.getChatMessages(room, Number(limit));
  ...
  // Travel post routes → server/modules/posts/routes.ts (registerPlatformModules)
